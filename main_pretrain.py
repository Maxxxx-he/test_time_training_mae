...
def main(args):
    ...
    transform_train = transforms.Compose([
        ...
        transforms.Normalize(mean=[0.676, 0.566, 0.664], std=[0.227, 0.253, 0.217])]) #SPECIAL IN IMAGENET1K
    ...
    # define the model
    model = models_mae_shared.__dict__[args.model](num_classes=num_classes, img_size=args.input_size,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with torch.no_grad(): #停止autograd模块的工作，以起到加速和节省显存的作用，具体行为就是停止gradient计算
            test_stats = evaluate(data_loader_val, model, device)
            if test_stats['acc1'] is not None:
