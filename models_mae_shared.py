...
class MaskedAutoencoderViT(nn.Module):
    ...
    #vit base#emb_dim=768 depth = 12
    #vit large default
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 embed_dim=1024, depth=24, num_heads=16,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 classifier_embed_dim=768, classifier_depth=12, classifier_num_heads=12,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, num_classes: int = 2, head_type: str = 'vit_head',
        ...
        self.patch_embed = PatchEmbed(img_size, patch_size, in_chans, embed_dim)  #patch_embedding_dim
        num_patches = self.patch_embed.num_patches
        self.head_type = head_type  ##
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))  #trainable
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim),
                                      requires_grad=False)  # fixed sin-cos embedding
        # requires_grad=False -> not trainable
        #num_patches + 1: cls require position also
        self.blocks = nn.ModuleList([
            Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer)  # mlp_ratio =4 usually
            for i in range(depth)])
        self.norm = norm_layer(embed_dim)  #encoder output normalization
        ...
        if not no_decoder:
            ...
            if not self.rotation_prediction:  ##
                self.mask_token = nn.Parameter(torch.zeros(1, 1, decoder_embed_dim))  #trainable
        ...
        if self.head_type == 'vit_head':
            ...
            self.classifier_pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, classifier_embed_dim),
                                                     requires_grad=False)  # fixed sin-cos embedding

            dpr = [x.item() for x in torch.linspace(0, drop_path_rate, classifier_depth)]
            self.classifier_blocks = nn.ModuleList([
                Block(classifier_embed_dim, classifier_num_heads, mlp_ratio, qkv_bias=True, norm_layer=norm_layer,
                      drop_path=dpr[i])
    ...
    def initialize_weights(self):
        ...
        if self.head_type == 'vit_head':  ##
            classifier_pos_embed = get_2d_sincos_pos_embed(self.classifier_pos_embed.shape[-1],
                                                           int(self.patch_embed.num_patches ** .5), cls_token=True)
        ...
        self.apply(self._init_weights)  #nn.module method accept an function
    ...
    def forward(self, imgs, target=None, mask_ratio: float = 0.75, input_mask=None, reconstruct=True):
        ...
        if reconstruct and (mask_ratio != 0):  #ssl
            pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]  64 196 16*16*3
            #print("------")
