...
def load_combined_model(args, num_classes: int = 1000):
    ...
    model = models_mae_shared.__dict__[args.model](num_classes=num_classes, head_type=args.head_type,
                                                   ...
                                                   classifier_num_heads=classifier_num_heads, rotation_prediction=False)
    ...
    # print("--------------------------")
    # for key in model_checkpoint['model']:
    #     print(key)
...
def main(args):
    ...
    max_known_file = max([int(i.split('results_')[-1].split('.npy')[0]) for i in
                          glob.glob(os.path.join(args.output_dir, 'results_*.npy'))] + [-1])
    if max_known_file != -1:
